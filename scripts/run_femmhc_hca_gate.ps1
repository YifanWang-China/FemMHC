...
if ([string]::IsNullOrWhiteSpace($OutputRoot)) {
    $OutputRoot = Join-Path $workspace ("artifacts\benchmark\femmhc-hca-gate-seed{0}" -f $Seed)
}
...
# Widths were selected once from the parameter count at seed 42.  The largest
# gap is below 3%, and every arm sees identical cached OpenMHC inputs, cohorts,
# splits, optimizer, budget, and final-step checkpoint selection.
$models = @(
    @{ Name = 'last_day_shared'; Hidden = 168 },
    @{ Name = 'shared_backbone'; Hidden = 166 },
    @{ Name = 'mmoe'; Hidden = 136 },
    @{ Name = 'dual_path_router'; Hidden = 128 },
    @{ Name = 'history_conditioned_adapter'; Hidden = 160 }
)
...
foreach ($model in $models) {
    ...
    if (-not (Test-Path -LiteralPath $checkpoint) -or -not (Test-Path -LiteralPath $summary)) {
        ...
        & $python (Join-Path $workspace 'scripts\train_femmhc_joint.py') `
            ...
            --seed $Seed `
            --device $Device 2>&1 | Tee-Object -FilePath (Join-Path $OutputRoot ("train-{0}.log" -f $name))
    ...
    if (-not (Test-Path -LiteralPath $metrics)) {
        ...
        & $python (Join-Path $workspace 'scripts\evaluate_femmhc_joint.py') `
            ...
            --split validation `
            --device $Device 2>&1 | Tee-Object -FilePath (Join-Path $OutputRoot ("evaluate-{0}.log" -f $name))
        if ($LASTEXITCODE -ne 0) { throw "Evaluation failed: model=$name seed=$Seed" }
    }
}

Write-Output ("[{0}] HCA single-seed gate complete: {1}" -f (Get-Date -Format o), $OutputRoot)
